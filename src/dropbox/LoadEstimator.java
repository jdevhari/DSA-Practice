 ...
 public static int[] solution(int[] sizes, int[] uploadingStart, int v) {
    ...
    while(!allDone){
        ...
        while(it.hasNext()){
            ...
            System.out.println("\t" + uploader + ":" + processMap.get(uploader) + ":" + ((double)v/coUploaderCount) + ":" + remain);
            if(remain <= 0){
                it.remove();
                result[uploader]= tick+1;
                System.out.println("\t\t" + uploader + " started at " + uploadingStart[uploader] + " finished at " + result[uploader]);
            }else {
